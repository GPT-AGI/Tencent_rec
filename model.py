...
class BaselineModel(torch.nn.Module):
    ...
    def __init__(self, user_num, item_num, feat_statistics, feat_types, args):  #
        ...
        if args.use_mixed_optimizer:
            ...
            self.pos_emb = torch.nn.Embedding(2 * args.maxlen + 1, args.hidden_units, padding_idx=0, sparse=True)

            for k in self.USER_SPARSE_FEAT:
                self.sparse_emb[k] = torch.nn.Embedding(self.USER_SPARSE_FEAT[k] + 1, args.hidden_units, padding_idx=0, sparse=True)
            for k in self.ITEM_SPARSE_FEAT:
                self.sparse_emb[k] = torch.nn.Embedding(self.ITEM_SPARSE_FEAT[k] + 1, args.hidden_units, padding_idx=0, sparse=True)
            for k in self.ITEM_ARRAY_FEAT:
                self.sparse_emb[k] = torch.nn.Embedding(self.ITEM_ARRAY_FEAT[k] + 1, args.hidden_units, padding_idx=0, sparse=True)
            for k in self.USER_ARRAY_FEAT:
                self.sparse_emb[k] = torch.nn.Embedding(self.USER_ARRAY_FEAT[k] + 1, args.hidden_units, padding_idx=0, sparse=True)
        else:
